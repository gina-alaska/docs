...
h2. Caching Issues
 The most common problem our users encounter is with the caching of the data returned by the OGC services.  the ESRI toolsets cache data returned from the OGC services and do not always delete or cleanup the cache when the service is removed, so subsequent requests don't use the OGC services, and hit the cache. i
